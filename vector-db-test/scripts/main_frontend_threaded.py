#!/usr/bin/env python3
"""
Быстрое решение: многопоточный веб-сервер для frontend
Замена однопоточного TCPServer на ThreadingTCPServer
"""
...
def start_remote_server(env="prod"):
    """Запуск многопоточного веб-сервера для frontend"""
    ...
    print("🚀 Запуск многопоточного веб-сервера...")
    print("🛡️ Защита от зависания: каждый запрос в отдельном потоке")
    ...
    try:
        # КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: используем ThreadingTCPServer вместо TCPServer
        # Это позволяет обрабатывать каждый запрос в отдельном потоке
        with socketserver.ThreadingTCPServer((HOST, PORT), CustomHTTPRequestHandler) as httpd:
            print(f"✅ Многопоточный сервер запущен и принимает соединения")
            print(f"🔍 Слушаю на {HOST}:{PORT}")
            print(f"🧵 Режим: многопоточный (ThreadingTCPServer)")
...
def main(env="prod"):
    """Главная функция"""
    print("=" * 60)
    print("🌍 МНОГОПОТОЧНЫЙ FRONTEND СЕРВЕР")
    print("=" * 60)
