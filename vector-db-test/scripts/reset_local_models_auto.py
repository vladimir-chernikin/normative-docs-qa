#!/usr/bin/env python3
"""
Скрипт для автоматической очистки и повторной загрузки локальных моделей.
Удаляет все локальные модели и запускает их загрузку заново без запроса подтверждения.
Используйте этот скрипт для автоматизации процесса или в CI/CD.
"""
...
def main():
    """Основная функция"""
    print("🔄 Автоматический сброс и повторная загрузка локальных моделей")
    ...
    if success:
        print("\n✅ Процесс сброса и повторной загрузки моделей завершен успешно")
    else:
        print("\n❌ Процесс сброса и повторной загрузки моделей завершен с ошибками")
        sys.exit(1)
